r"""Hyperbolic geometry utilities.
...
def full_turn_deg(r: float, kappa: float) -> float:
    r"""Return degrees in one revolution at hyperbolic radius ``r``.
...
def rotate_cmd(delta_deg_a: float, r: float, kappa: float) -> float:
    r"""Convert a rotation request in adaptive degrees to Euclidean radians."""
    d_full = full_turn_deg(r, kappa)
    if d_full == 0:
        return math.radians(delta_deg_a)
    frac = delta_deg_a / d_full
    return frac * 2 * math.pi


def pi_a_over_pi(r: float, kappa: float) -> float:
    r"""Return the ratio ``\pi_a/\pi`` at radius ``r``."""
    if r == 0:
        return 1.0
